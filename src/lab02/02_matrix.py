...
import is_matrix # модуль проверяющий является ли матрица рванной
...
for case_mat in transpose_case_mat: print(transpose(case_mat))
print('-' * 20)
...
def row_sums(mat: list[list[float | int]]): 
    state_of_matrix = is_matrix.is_matrix(mat)
    if state_of_matrix != ValueError: return [sum(string_matrix) for string_matrix in mat]
    else: return ValueError('ValueError: рванная матрица')

row_sums_case_list = [[[1, 2, 3], [4, 5, 6]], [[-1, 1], [10, -10]], [[0, 0], [0, 0]], [[1, 2], [3]]]
for case_mat in row_sums_case_list: print(row_sums(case_mat))
print('-' * 20)
...
col_sums_case_mat = [[[1, 2, 3], [4, 5, 6]], [[-1, 1], [10, -10]], [[0, 0], [0, 0]], [[1, 2], [3]]]
for case_mat in col_sums_case_mat: print(col_sums(case_mat))
print('-' * 20)