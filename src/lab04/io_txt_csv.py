...
def sorted_word_counts_per_file(
    freq: dict[str, int], file_name
) -> list[tuple[str, int]]:
    return [
...
def analize_txt_per_file(path):
    ...
    for file in all_files:
        files_sort = files_sort + sorted_word_counts_per_file(
            frequencies_from_text(read_text(file)), file.name
        )
    return files_sort
