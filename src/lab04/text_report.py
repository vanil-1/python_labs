...
def sorted_word_per_file(freq: dict[str, int], file_name) -> list[tuple[str, int]]:
    return [
        (file_name, pos[0], pos[1])
        for pos in sorted(freq.items(), key=lambda kv: (-kv[1], kv[0]))
    ]


def sorted_word_total(freq: dict[str, int]) -> list[tuple[str, int]]:
...
def analize_txt_per_file(path, txt=[]):
    ...
    for file in all_files:
        txt += sorted_word_per_file(freq_from_text(read_text(file)), file.name)
    return txt
...
def text_csv_duo(path_repositories, name_of_csv_file, header):
    txt_per_file, txt_total = analize_txt_per_file(
        path_repositories
    ), analize_txt_total(path_repositories)
    write_csv(
        txt_per_file, f"data/lab04/{name_of_csv_file}_report_per_file.csv", header
    )
    if header:
        write_csv(
            txt_total,
            f"data/lab04/{name_of_csv_file}_report_total.csv",
            (header[1], header[2]),
        )
    else:
        write_csv(txt_total, f"data/lab04/{name_of_csv_file}_report_total.csv", None)
    print(f"Всего слов: {len(txt_per_file)}")
    print(f"Уникальных слов: {len(txt_total)}")
    print("Топ-5:")
    for word_top_5 in txt_total[:5]:
