...
def sorted_word_counts_total(freq: dict[str, int]) -> list[tuple[str, int]]: return sorted(freq.items(), key=lambda kv: (-kv[1], kv[0]))
...
def analize_txt_total(path):
    txt = ''
    for file in Path(path).glob('*txt'): txt = txt + read_text(file)
    return sorted_word_counts_total(frequencies_from_text(txt))
