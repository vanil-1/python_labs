...
def csv_to_json(csv_path: str, json_path: str) -> None:
    csv_path, json_path = pathProccesing(csv_path, False, ".csv"), pathProccesing(
        json_path, True, ".json"
    )
    try:
        with json_path.open("w", newline="", encoding="utf-8") as f_json, csv_path.open(
            encoding="utf-8"
        ) as f_csv:
...
def json_to_csv(json_path: str, csv_path: str) -> None:
    json_path, csv_path = pathProccesing(json_path, False, ".json"), pathProccesing(
        csv_path, True, ".csv"
    )
    try:
        with csv_path.open("w", newline="", encoding="utf-8") as f_csv, json_path.open(
            encoding="utf-8"
        ) as f_json:
...
def csv_to_xlsx(csv_path: str, xlsx_path: str) -> None:
    csv_path, xlsx_path = pathProccesing(csv_path, False, ".csv"), pathProccesing(
        xlsx_path, True, ".xlsx"
    )
    try:
