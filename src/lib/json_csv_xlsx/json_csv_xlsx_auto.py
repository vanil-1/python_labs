...
def json_csv_xlsx(path_in: str, path_out: str, path_suffix = ('.json', '.csv', '.xlsx')) -> None:
    path_in, path_out = pathProccesing(path_in, False, path_suffix), pathProccesing(path_out, True, path_suffix)
    path_in_suffix, path_out_suffix = path_in.suffix.lower(), path_out.suffix.lower()
    try:
        with path_out.open("w", newline = "", encoding = "utf-8") as f_out, path_in.open(encoding = "utf-8") as f_in:
